...
public class GithubEnterpriseLoginActivity extends AccountAuthenticatorActivity {
  ...
  @Bind(R.id.enterpriseUrl) EditText enterpriseUrl;
  @Bind(R.id.accessToken) EditText enterpriseToken;
  ...
  public void onLogin() {
    if (enterpriseUrl.length() > 0 && enterpriseToken.length() > 0) {
      ...
      if (Fabric.isInitialized()) {
        Answers.getInstance().logLogin(new LoginEvent().putMethod("enterprise").putSuccess(true));
      }
