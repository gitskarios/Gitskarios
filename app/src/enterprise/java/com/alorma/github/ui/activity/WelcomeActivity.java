...
public class WelcomeActivity extends AccountAuthenticatorActivity implements GithubLoginFragment.LoginCallback {
  ...
  @Bind(R.id.buttonEnterprise) Button buttonEnterprise;

  @Bind(R.id.buttonOpen) Button buttonOpen;

  @Bind(R.id.importAccountsSwitch) CompoundButton importAccountsSwitch;
  ...
  private void openCreateEnterprise() {
    buttonGithub.setVisibility(View.INVISIBLE);
    buttonEnterprise.animate().alpha(0f).setDuration(TimeUnit.SECONDS.toMillis(1)).setListener(new Animator.AnimatorListener() {
      @Override
      public void onAnimationStart(Animator animation) {
  ...
  private void addAccount(User user) {
    if (ContextCompat.checkSelfPermission(this, "android.permission.AUTHENTICATE_ACCOUNTS") == PackageManager.PERMISSION_GRANTED) {
