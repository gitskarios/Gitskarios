...
public class NotificationsSyncAdapter extends AbstractThreadedSyncAdapter {
  ...
  public void onPerformSync(Account account, Bundle extras, String authority,
    ...
    if (token != null) {
      ...
      notificationsClient.observable()
          ...
          .subscribe(new NotificationsSubscriber(account, token));
    }
  }

  private void onNotificationsReceived(List<Notification> notifications, Account account,
      String token) {
    if (notifications != null) {
      ...
      for (Notification notification : notifications) {
        boolean showNotification =
            NotificationsHelper.checkNotFireNotification(getContext(), notification.id);
        if (showNotification) {
  ...
  private NotificationCompat.Builder createNotificationBuilder(Repo repository, String account,
    ...
    int shape_notifications_avatar =
        getContext().getResources().getDimensionPixelOffset(R.dimen.shape_notifications_avatar);
  ...
  private void fireSingleNotifications(String account, String token,
    ...
    if (githubNotification != null) {
      ...
      PendingIntent pendingIntent =
          PendingIntent.getActivity(getContext(), 0, intentNotificationsActivity, 0);

      Intent intentDisableService =
          NotificationsDisableService.createIntentSingleNotification(getContext(),
              githubNotification);
      PendingIntent cancelIntent =
          PendingIntent.getService(getContext(), (int) githubNotification.id, intentDisableService,
