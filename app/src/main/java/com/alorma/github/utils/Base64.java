...
class Base64 {
   ...
   */
  private static void encode3to4(byte[] source, int srcOffset, int numSigBytes, byte[] destination,
      int destOffset) {
  ...
  private static int decode4to3(byte[] source, int srcOffset, byte[] destination, int destOffset) {
    ...
    if (source[srcOffset + 2] == EQUALS_SIGN) {
      int outBuff =
          ((DEC[source[srcOffset]] & 0xFF) << 18) | ((DEC[source[srcOffset + 1]] & 0xFF) << 12);
    ...
    // Example: DkLE
      ...
      int outBuff =
          ((DEC[source[srcOffset]] & 0xFF) << 18) | ((DEC[source[srcOffset + 1]] & 0xFF) << 12) | ((
              DEC[source[srcOffset + 2]]
                  & 0xFF) << 6) | ((DEC[source[srcOffset + 3]] & 0xFF));
    ...
    for (int i = off; i < off + len; i++) {
      ...
      if (EQUALS_SIGN_DEC <= sbiDecode) {
        ...
        throw new IllegalArgumentException(
            MessageFormat.format("Bad Base64 input character at {0} : {1} (decimal)", i,
                source[i] & 0xff));
      }
    }
